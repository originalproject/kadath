...
describe Boxes::Pd do

  it "can be initialized with a Pd object string" do
    pdbox = Boxes::Pd.new("foo")
    pdbox.must_be_instance_of Boxes::Pd
    pdbox.pd_object.must_equal "foo"
  end

  it "must be initialized with a parameter" do
    -> { pdbox = Boxes::Pd.new(nil) }.must_raise RuntimeError
  end
  ...
  it "has an id which is unique" do
    pdb1 = Boxes::Pd.new("foo")
    pdb2 = Boxes::Pd.new("bar")
